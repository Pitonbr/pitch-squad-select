-- Check and add geographic fields to teams table only if they don't exist
DO $$ 
BEGIN 
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'teams' AND column_name = 'public_description') THEN
        ALTER TABLE public.teams ADD COLUMN public_description TEXT;
    END IF;
...
-- Create team join requests table if it doesn't exist
...
DROP POLICY IF EXISTS "Requesting players can view their own requests" ON public.team_join_requests;
DROP POLICY IF EXISTS "Team admins can view team join requests" ON public.team_join_requests;
DROP POLICY IF EXISTS "Authenticated users can create join requests" ON public.team_join_requests;
DROP POLICY IF EXISTS "Team admins can update join requests" ON public.team_join_requests;
...
AS $$
  ...
  ORDER BY 
    CASE WHEN LOWER(t.name) LIKE LOWER('%' || COALESCE(_search_term, '') || '%') THEN 1 ELSE 2 END,
    member_count DESC,
    t.created_at DESC
  LIMIT _limit;
$$;
